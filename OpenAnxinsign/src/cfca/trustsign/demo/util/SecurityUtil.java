...
public class SecurityUtil {
    ...
    public static String getOpenPlatSignData(String bodyData, String openTs) throws PKIException {
        String unSignData = "body=" + bodyData + "&X-OPEN-TS=" + openTs, hashData = hashDataForOpenPlat(unSignData);
        System.out.println("unSignData:" + unSignData);
        System.out.println("hashData:" + hashData);
        return p7SignMessageDetach(HttpConnector.JKS_PATH_OP_USER, HttpConnector.JKS_PWD_OP_USER, HttpConnector.ALIAS_OP_USER, hashData);
    }

    public static String hashDataForOpenPlat(String signData) throws PKIException {
        byte[] hashResult;
        try {
            hashResult = HashUtil.RSAHashData(signData.getBytes(SystemConst.DEFAULT_CHARSET), new Mechanism(Mechanism.SHA256), session, false);
            return Hex.toHexString(hashResult).toLowerCase();
        } catch (Exception e) {
            throw new PKIException("hash data for open plat fail", e);
        }
    }
